...
def book_dct_postgres_serial(db, retry_max_sleep=10.0, verbose=1):
    ...
    """
    print (sys.stderr) #"""# TOD: use the priority field, not the status."""
    print (sys.stderr) #"""#TOD: ignore entries with key PUSH_ERROR."""
    ...
    while (dct is None) and keep_trying:
        ...
        try:
            ...
            if dct is not None:
                ...
                # no jobs are left
                keep_trying = False
        except (sqlalchemy.exc.DBAPIError, CONCURRENT_ERROR) or e:
