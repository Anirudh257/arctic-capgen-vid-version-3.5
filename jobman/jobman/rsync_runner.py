...
# some site said that this number could (should) be really high like 10K
HASH_REPS = 2000
        ...
        try:
            try:
                ...
                if srclist:
                    ...
                    os.rename(
                        os.path.join(
                            os.path.join(self.exproot, self.todo, choice)),
...
def run_callback_in_rsynced_tempdir(remote_rsync_loc, callback,
    ...
    rsync(remote_rsync_loc, tmpdir, exclusions=[
          '*.no_sync_to_client', '*.no_sync'])
    ...
    try:
        ...
        if status_filename:
            ...
            print("%(now)s Running %(callbackname)s in %(localhost)s:%(tmpdir)s" % locals(
            ), file=statusfile)
        rsync(tmpdir, remote_rsync_loc, exclusions=[
              '*.no_sync_to_server', '*.no_sync'])
        ...
        if status_filename:
            now = str(datetime.datetime.now())
            print("%(now)s Done %(callbackname)s in %(localhost)s:%(tmpdir)s" % locals(
            ), file=statusfile)
            statusfile.flush()
