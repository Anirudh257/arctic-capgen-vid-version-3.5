...
def load_pkl(path):
    ...
    try:
        # rval = cPickle.load(f)
        if "CAP.pkl" or "FEAT_key_vidID_value_features.pkl" or "worddict.pkl" in path:
            rval = load_pickle(path)
        else:
        	rval = cPickle.load(f)
    finally:
        f.close()
...
def load_pickle(pickle_file):
	...
	"""    
	# Reference: https://blog.modest-destiny.com/posts/python-2-and-3-compatible-pickle-save-and-load/
	try:
		with open(pickle_file, 'rb') as f:
			pickle_data = cPickle.load(f)
	except UnicodeDecodeError as e:
		with open(pickle_file, 'rb') as f:
			pickle_data = cPickle.load(f, encoding = 'latin1')
...
def generate_minibatch_idx(dataset_size, minibatch_size):
    ...
    # print("n_minibatches: " + str(n_minibatches))
